...
@Component({
  ...
  template: `
    <div class="page-hero"><div class="ph"><h1>Artículos de Profesores</h1><p>Producción científica del plantel docente</p></div></div>
    <section class="section"><div class="container">
      @for (art of articulos(); track art.key || $index) {
      <div class="art-card card-usco" style="padding:1.5rem;margin-bottom:1rem;display:flex;gap:1rem;align-items:flex-start;transition:transform .2s">
...
export class ArticulosDocentesComponent implements OnInit {
  ...
  ngOnInit() {
    this.fs.getCollection<Articulo>('articulosDocentes').subscribe(d => this.articulos.set(d || []));
  }
}
