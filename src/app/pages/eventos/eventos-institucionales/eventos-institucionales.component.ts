...
@Component({
  selector: 'app-eventos-institucionales', standalone: true, imports: [CommonModule, Tag],
  template: `
    <div class="page-hero"><div class="ph"><h1>Actividades Institucionales</h1><p>Eventos de la Universidad Surcolombiana</p></div></div>
...
export class EventosInstitucionalesComponent implements OnInit {
  private fs = inject(FirestoreService);
  eventos = signal<Evento[]>([]);
  ngOnInit() {
    this.fs.getCollection<Evento>('eventosInstitucionales').subscribe(d => this.eventos.set(d || []));
  }
}
